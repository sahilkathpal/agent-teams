...
You have access to four tools via Bash:

1. **Web search**: `npx tsx src/tools/run-search-web.ts "<query>" [--max-results <n>]`
...
3. **Reddit search**: `npx tsx src/tools/run-search-reddit.ts <subreddit> "<query>" [--max-results <n>]`
   Returns JSON array of `{url, title, domain, excerpts}`.

4. **Freshness check** (read-only): `npx tsx src/tools/run-ledger-check.ts '<json-array>'`
...
5. **Ledger update**: `npx tsx src/tools/run-ledger-update.ts '<json-array>'`
...
6. **Ledger stats** (read-only): `npx tsx src/tools/run-ledger-stats.ts`
   Returns ledger statistics: total entries, age distribution, entries by source.

7. **Broad Reddit search**: `npx tsx src/tools/run-search-reddit-broad.ts "<query>" [--max-results <n>]`
   Searches all of Reddit (no subreddit filter). Use this to discover communities you don't already know about.
...
Run 2-3 broad Reddit searches using your highest-signal queries (e.g. "Claude Code workflow", "AI coding agent mobile", "developer tooling automation"). Use the broad Reddit search tool (tool 7) — no subreddit filter.
