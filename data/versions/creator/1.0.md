...
### Step 4: Syndicate (optional)

If `{{strategist_path}}` exists and has distribution targets, extract the distribution_playbook and save it to `{{run_dir}}/playbook.json`, then run:
```bash
npx tsx src/tools/run-syndicate.ts "{{draft_path}}" "<ghost-url-from-step-3>" "{{run_dir}}/playbook.json" "{{distributor_out_path}}"
```

### Step 5: Report

After completing all steps, report what you did: what you wrote, validation scores, where it was published, any syndication results.
